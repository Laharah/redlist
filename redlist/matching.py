...
class TrackInfo:
    ...
    def _clean_feat(self):
        ...
        for f in fields:
            m = re.match(
                r"(.+?)[\s\-]+(, |feat(\. |uring )?)(.+)", self.__getattribute__(f)
            )
            if m:
    ...
    def from_spotify(cls, track):
        ...
        try:
            ...
            raise MatchingError("Trouble Matching Spotify Track.", data=track) from e
        return cls(**kwargs)
...
def beets_match(track_info, lib, restrict_album=False):
    ...
    for t in track_info:
        ...
        if not res:
            ...
            canidates = {
                track_distance(r, t, restrict_album=restrict_album): r for r in res
            }
